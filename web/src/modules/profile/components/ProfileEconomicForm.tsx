...
interface ProfileEconomicFormProps {
  formData: any
  onChange: (field: string, val: any) => void
}

const CASTES = ['General', 'OBC', 'SC', 'ST', 'EWS']
const OCCUPATIONS = ['unemployed', 'farmer', 'student', 'employed', 'self-employed', 'daily-wage', 'senior-citizen']
const MARITAL_STATUSES = ['Single', 'Married', 'Widowed', 'Divorced']

export function ProfileEconomicForm({ formData, onChange }: ProfileEconomicFormProps) {
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 shadow-xl space-y-4">
      <div className="flex items-center gap-3 pb-3 border-b border-zinc-800">
        ...
        <div>
          <h3 className="text-sm font-bold text-white">Socio-Economic & Household Factors</h3>
          <p className="text-xs text-zinc-400">Income, caste category, employment, and special criteria</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 text-xs">
        <label className="flex items-center gap-3 p-3.5 bg-zinc-800/50 rounded-2xl border border-zinc-800 cursor-pointer text-zinc-300 hover:bg-zinc-800">
          <input
            type="checkbox"
          ...
          <div>
            ...
            <span className="text-[11px] text-zinc-400">Qualifies for farmer DBT and subsidy schemes</span>
          </div>
        </label>
        ...
        <label className="flex items-center gap-3 p-3.5 bg-zinc-800/50 rounded-2xl border border-zinc-800 cursor-pointer text-zinc-300 hover:bg-zinc-800">
          ...
          <div>
            ...
            <span className="text-[11px] text-zinc-400">Qualifies for special disability assistance grants</span>
          </div>
        </label>
      </div>
    </div>
