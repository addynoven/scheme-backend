  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        console.warn('Better Auth signup fallback:', betterAuthErr)
      }

      // 2. Register with FastAPI backend
      await citizenRegister({ email, phone, password })

      // 3. Auto Login to obtain access token
      ...
      <div className="rounded-3xl bg-zinc-900/80 border border-zinc-800/80 p-6 sm:p-8 shadow-2xl backdrop-blur-xl">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <button
            ...
            className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-zinc-950 hover:bg-zinc-800 border border-zinc-800 hover:border-zinc-700 text-xs font-semibold text-zinc-200 transition-all cursor-pointer disabled:opacity-50"
          >
