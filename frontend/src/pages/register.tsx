...
import { citizenLogin, citizenRegister } from '@/lib/api'
import { setCitizenToken } from '@/lib/session'
import { ShieldCheck, ArrowRight, Lock, Mail, Phone, AlertCircle, Sparkles } from 'lucide-react'
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Auto Login to obtain access token
      const authRes = await citizenLogin(email, password)
      if (authRes.access_token) {
        setCitizenToken(authRes.access_token)
        navigate('/profile')
      } else {
        navigate('/login')
      }
      ...
      <div className="rounded-3xl bg-zinc-900/80 border border-zinc-800/80 p-6 sm:p-8 shadow-2xl backdrop-blur-xl">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-2 py-3 px-4 rounded-xl bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 hover:from-emerald-500 hover:via-teal-500 hover:to-cyan-500 text-white font-semibold text-sm shadow-lg shadow-emerald-600/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-50"
          >
            {loading ? (
              <div className="h-5 w-5 border-2 border-white/20 border-t-white rounded-full animate-spin" />
            ) : (
              <>
                <span>Issue Citizen ID & Continue</span>
        ...
        <div className="mt-6 pt-6 border-t border-zinc-800/80">
          <button
            ...
            className="w-full py-2.5 px-3 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 text-xs font-semibold text-emerald-300 transition-colors flex items-center justify-center gap-2"
          >
            <Sparkles className="h-3.5 w-3.5 text-emerald-400" />
            <span>Generate Random Test Citizen Credentials</span>
          </button>
        </div>

        <div className="mt-6 text-center text-xs text-zinc-400">
          <span>Already have a Citizen ID? </span>
